...
def underlined_sentences(img):
    ...
    kernel = np.ones((1,40), np.uint8) # A horizontal line kernel for line detection

    morphed = cv2.morphologyEx(gray, cv2.MORPH_CLOSE, kernel) # Dilation followed by Erosion
    ...
    # Create the imgs that will use to extract the horizontal and vertical lines
    ...
    contours, hierarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # Finding the Contours in the thresh image
    ...
    sections = [] # A list containing row numbers of continuous lines of all sections

    each_section = [] # A list containing row numbers of continuous lines of each section

    for i in range(0, len(jss) - 1):
        if abs(jss[i] - jss[i + 1]) < 20:
            each_section.append(jss[i]) # If the difference of the row numbers of continuous lines is less than 20 means they are a part of one section
    ...
    for i in range(0, len(final_sections)):
        ...
        average_height = AvgHeight(filename1 = 'cropped_img1.jpeg') # Used to calculate the average height of a sentences for better ROI detection
